...
public class HojaSubrogacionServiceImpl  implements HojaSubrogacionService {
	...
	public Response<Object> consultarFolioOrden(DatosRequest request, Authentication authentication) throws IOException {
		...
		try {
			...
			response = providerServiceRestTemplate.consumirServicio(new FolioOrdenServicio().obtenerFolios(request, usuarioDto, folioRequest).getDatos(), 
					urlDominio.concat(AppConstantes.CATALOGO_CONSULTAR), authentication);
			consulta =new FolioOrdenServicio().obtenerFolios(request, usuarioDto, folioRequest).getDatos().get(AppConstantes.QUERY).toString();
